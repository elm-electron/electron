effect module Electron.IpcRenderer where { subscription = MySub, command = MyCmd } exposing
  ( on
  , send
  )
...
onWatcherEffects router newSubs oldState =
  let
    ...
    bothStep eventName watcher decoders task =
      ...
      Task.succeed
        (Dict.insert eventName ({ watcher | decoders = decoders }) state)
    ...
    rightStep eventName decoders task =
      ...
      Task.succeed
        (Dict.insert eventName (Watcher decoders pid) state)
  in
    Dict.merge
      leftStep
      bothStep
